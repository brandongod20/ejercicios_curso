...
class App(customtkinter.CTk):
    ...
    def btn_calcular_on_click(self):
        ...
        calcularMC = int(ancho) * int(largo)
        calcularML = int(ancho) + int(largo) + int(largo) + int(ancho)

        alert(title="Metros cuadrados",message="son " + str(calcularMC) + " Metros cuadrados")
        alert(title="Metros lineales",message="Son " + str(calcularML) + " Metros lineales")
     
     # B. Informar la cantidad de postes de quebracho Grueso de 2.4 mts (van cada 250 mts lineales y en las esquinas).
        ancho= self.txt_ancho.get()
        largo= self.txt_largo.get()

        calcularPQG = calcularML // 250

        alert(title="Postes quebrachos Grueso",message="Hay " + str(calcularPQG) + " Postes quebrachos de 2.4 mts" ) 
    # C. Informar la cantidad de postes de quebracho Fino de 2.2 mts (van cada 12 mts lineales, si en es lugar no se encuentra el poste grueso).
        ancho= self.txt_ancho.get()
        largo= self.txt_largo.get()
        
        calcularPQF = calcularML // 12

        alert(title="postes quebrachos Finos",message="Hay " + str(calcularPQF) + " Postes quebrachos de 2.2 mts") 
    # D. Informar la cantidad de varillas (van cada 2 mts lineales). 
        ancho= self.txt_ancho.get()
        largo= self.txt_largo.get() 

        calcularVarillas = calcularML // 2

        alert(title="varillas", message="Hay " + str(calcularVarillas) + " varillas")
    # E. Informar la cantidad de alambre alta resistencia 17/15 considerando 7 hilos.
        ancho= self.txt_ancho.get()
        largo= self.txt_largo.get() 

        calcularAlambre = calcularMC // 7

        alert(title="cantidad de alambre", message="Hay " + str(calcularAlambre) + " metros de alambre")
